...
//
        ...
        GClass.setBounds(359,19,253,48);
        GClass.setForeground(new Color(250, 250, 250,250));
        add(GClass);
        Font GrClass = new Font ("Segoe UI",Font.BOLD,36);
        GClass.setFont(GrClass);

        JLabel Uname = new JLabel ("R R W M Ashen Nirmal Wadugodapitiya");
        Uname.setBounds(366,66,282,20);
...
//       box 5  Balance
        ...
        JLabel b5l2 = new JLabel ("0000.00");
...
//       box 6  Saving
        ...
        JLabel b6l1 = new JLabel ("Saving");
        b6l1.setBounds(491,399,70,24);
        b6l1.setForeground(new Color(0, 0, 0,250));
        add(b6l1);
        b6l1.setFont(b5l11);

        JLabel b6l2 = new JLabel ("0000.00");
        b6l2.setBounds(757,417,119,24);
        b6l2.setForeground(new Color(0, 0, 0,250));
        add(b6l2);
        b6l2.setFont(b5l11);

        JLabel b6 = new JLabel();
        b6.setBounds(469,393,427,64);
        b6.setIcon(new ImageIcon(getClass().getResource("Home/Rectangle 327.png")));
        add(b6);
        ...
        b6.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) {
                boxoutline.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
                boxoutline.setIcon(new ImageIcon(getClass().getResource("Home/kljkklkjl.png")));
                boxoutline.setBounds(469,393,427,64);
            }
        });
...
//------------------------------------------------------------------------------------------------------------------------------
        try{
            ...
            ResultSet rs = st.executeQuery("SELECT * FROM accounts  WHERE  accounts_name = '"+b5l1.getText()+ "' &&  user_nic = "+Integer.parseInt(UID));
            while(rs.next()){
                String ag= rs.getString("amount");
                b5l2.setText(ag);
            }
            ResultSet rss = st.executeQuery("SELECT * FROM accounts   WHERE  accounts_name = '"+b6l1.getText()+ "' &&  user_nic = "+Integer.parseInt(UID));
            while(rss.next()){
                String ag= rss.getString("amount");
                b6l2.setText(ag);
            }
        }
        catch(Exception ew){
...
//------------------------------------------------------------------------------------------------------------------------------
        try{
            ...
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost/online_banking_system","root","");
            Statement st = (Statement) con.createStatement ();
            ResultSet rss = st.executeQuery("SELECT * FROM user_register  WHERE user_nic = "+Integer.parseInt(UID));

            while(rss.next()){
                String ag= rss.getString("full_name");
                Uname.setText(ag);
            }
        }
        catch(Exception ew){
